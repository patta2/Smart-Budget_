...
// ตรวจสอบและหักบิลรายจ่ายประจำอัตโนมัติ (ข้อ 5)
...
function renderHome() {
  ...
  let budget = Math.max(0, Number(state.allowance) || 0), used = Math.max(0, t.expense);
  let percent = budget > 0 ? (used / budget) * 100 : 0;
  let status = percent >= 100 ? 'over' : percent >= 90 ? 'low' : percent >= 70 ? 'near' : 'plan';
...
function renderRecurring() {
  ...
  if ($('recurringList')) {
    $('recurringList').innerHTML = state.recurring.length ? state.recurring.map((b, i) => { 
      ...
      return '<div class="list-item"><div class="cat-icon"><i data-lucide="repeat"></i></div><div class="list-main"><b>' + esc(b.name) + '</b><p class="tiny muted money">' + money(b.amount) + ' (วันที่ ' + (b.day || 1) + ')' + (paid ? ' · ชำระแล้ว' : '') + '</p></div><button class="' + (paid ? 'secondary' : 'primary') + '" type="button" ' + (paid ? 'disabled' : 'onclick="payRecurring(' + i + ')"') + '>' + (paid ? 'ชำระแล้ว' : 'จ่าย') + '</button></div>'; 
...
function addRecurringBill() {
  ...
  if (!list) return;
  list.innerHTML = '<form id="recurringForm" class="field"><label>ชื่อบิลรายจ่าย</label><input id="recName" class="input" required placeholder="ค่าเน็ต"><label>จำนวนเงิน</label><input id="recAmount" class="input" type="number" step="any" placeholder="300"><label>วันที่ต้องจ่ายประจำทุกเดือน (1-31)</label><input id="recDay" class="input" type="number" min="1" max="31" value="1"><button class="primary full" type="submit" style="margin-top:8px">บันทึกบิล</button></form>' + list.innerHTML;
...
// แยกหมวดหมู่เด็ดขาดระหว่าง รายรับ / รายจ่าย (ข้อ 9)
